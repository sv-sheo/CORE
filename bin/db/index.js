...
exports.wait_for_DB_ready = async function({config={}, CONN}={}) {
    ...
    try {
        ...
        var ready_result    = await DB.db(data.db).wait({waitFor: 'all_replicas_ready', timeout: 30}).run(CONN) // wait for max. 30 seconds for DBs to load their replicas; 0 = no timeout
